...
def test_v1_rc_cutover_gate_blocks_release_until_p0_evidence_passes() -> None:
    ...
    assert len(gate["failed_gates"]) == 2
...
def test_v1_rc_cutover_gate_markdown_is_release_operator_focused() -> None:
    ...
    assert "| Claude Code | `first_10_minutes_replay` | `blocked` | `2026-06-28` |" in markdown
...
def test_committed_v1_rc_cutover_gate_is_current() -> None:
    ...
    assert "[V1_RC_CUTOVER_GATE.md](V1_RC_CUTOVER_GATE.md)" in Path("docs/ARCHIVE.md").read_text(encoding="utf-8")
