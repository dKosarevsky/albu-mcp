...
Current release target: `v1.20.0`
...
- PyPI package target: `albumentationsx-mcp==1.20.0`
- MCP Registry target: `io.github.dKosarevsky/albu-mcp` version `1.20.0`
- GitHub Release target: Python artifacts plus `albumentationsx-mcp-1.20.0.mcpb`, the byte-identical
...
```bash
uv run python scripts/check_release_readiness.py --tag v1.20.0
...
```bash
uv run python scripts/check_published_package_smoke.py --version 1.20.0
