...
Current release target: `v1.21.1`
...
- MCP Registry target: `io.github.dKosarevsky/albu-mcp` version `1.21.1`
- GitHub Release target: Python artifacts plus `albumentationsx-mcp-1.21.1.mcpb`, the byte-identical
...
```bash
uv run python scripts/check_release_readiness.py --tag v1.21.1
...
```bash
uv run python scripts/check_published_package_smoke.py --version 1.21.1
