...
| Review Decision | Convert subjective preview review into structured, reproducible tuning actions. | `plan_preview_review`, `record_preview_feedback`, `export_preview_report` | `tests/fixtures/snapshots/output_contracts.json`, `docs/USAGE.md` | User feedback maps to tags, a next tool, and an auditable tuning decision. | Use `plan_preview_review` before adjustment, audit, or export. |
