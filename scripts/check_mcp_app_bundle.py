...
_REMOTE_CSS_PATTERN = re.compile(r"(?:@import\s+|url\(\s*['\"]?)(?:https?:)?//", re.IGNORECASE)
_REMOTE_SCRIPT_PATTERN = re.compile(
    r"\b(?:fetch|importScripts)\s*\(\s*['\"`](?:https?:)?//",
    re.IGNORECASE,
)
...
class _AppHTMLInspector(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        for name in _ACTIVE_URL_ATTRIBUTES:
            ...
            if value is not None and _contains_remote_reference(value):
...
def check_distributions(dist_dir: Path) -> dict[str, dict[str, Any]]:
    ...
    if len(wheels) != 1 or len(sdists) != 1:
        ...
        raise McpAppBundleError(msg)
    return {"wheel": inspect_wheel(wheels[0]), "sdist": inspect_sdist(sdists[0])}
...
def _validate_app_html(html_bytes: bytes, *, source: str) -> None:
    ...
    if inspector.script_count != 1 or not inspector.inline_script_text.strip():
        ...
        raise McpAppBundleError(msg)
    if _REMOTE_CSS_PATTERN.search(html) or _REMOTE_SCRIPT_PATTERN.search(inspector.inline_script_text):
        msg = f"MCP App in {source} contains an active remote dependency"
        raise McpAppBundleError(msg)


def _contains_remote_reference(value: str) -> bool:
    return any(_REMOTE_URL_PATTERN.match(part.strip()) for part in value.split(","))
