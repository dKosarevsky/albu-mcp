    ...
    return "\n".join(
        [
            ...
            *(_host_config_section(host=host, args=args) for host in config.hosts),
            ...
            "",
            "Do not commit private images, screenshots, prompts, absolute home paths, tokens, or host logs.",
            "Use redacted excerpts that prove the host behavior without exposing dataset content or local secrets.",
            "",
            "### Minimal Evidence Bundle",
            "",
            "- `host`: exact supported host name.",
            ...
            "### Status Rules",
            "",
            "- Record passed only when the real host UI completes the flow.",
            "- Record blocked when the host UI fails before evidence can be captured; name the first failing gate.",
            "- Keep pending when the host was not tested.",
            "",
            "## Evidence Checklist",
            "",
